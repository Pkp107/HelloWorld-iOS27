...
private struct ImportedRuntimeView: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            Text("Ready for LiveContainer")
                .font(.title2.weight(.bold))
                .multilineTextAlignment(.center)
            Text("\(app.displayName) is stored in HelloOS. The native LiveContainer runtime is not embedded in this target yet, so this IPA is managed but not executed here.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 330)
            Label(app.status.label, systemImage: "checkmark.circle")
